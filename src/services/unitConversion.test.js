...
describe('unitConversion', () => {
  ...
  describe('convertToMm', () => {
    ...
    it('should throw error for unknown unit', () => {
      expect(() => convertToMm(100, 'cm')).toThrow('Unknown unit: cm');
    });
  });
  ...
  describe('convertFromMm', () => {
    ...
    it('should throw error for unknown unit', () => {
      expect(() => convertFromMm(100, 'cm')).toThrow('Unknown unit: cm');
    });
  });
  ...
  describe('validateDimension', () => {
    it('should validate positive dimensions within range', () => {
      ...
      expect(validateDimension(500)).toEqual({ valid: true });
    });
    ...
    it('should allow zero when specified', () => {
      expect(validateDimension(0, { allowZero: true })).toEqual({ valid: true });
    });

    it('should reject values below minimum', () => {
      const result = validateDimension(5);
      expect(result.valid).toBe(false);
      expect(result.error).toContain('at least');
    });

    it('should reject values above maximum', () => {
      const result = validateDimension(1500);
      expect(result.valid).toBe(false);
      expect(result.error).toContain('not exceed');
    });

    it('should use custom min/max when specified', () => {
      expect(validateDimension(75, { min: 50, max: 100 })).toEqual({ valid: true });
      expect(validateDimension(25, { min: 50, max: 100 }).valid).toBe(false);
      expect(validateDimension(125, { min: 50, max: 100 }).valid).toBe(false);
    });

    it('should include formatted dimension in error messages', () => {
      const result = validateDimension(5, { min: 10 });
