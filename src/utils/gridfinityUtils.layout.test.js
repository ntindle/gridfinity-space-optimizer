...
describe('gridfinityUtils - Layout Logic', () => {
  describe('Real-world drawer scenarios', () => {
    it('should calculate correct layout for 22.5" x 16.5" drawer (standard US drawer)', () => {
      ...
      const drawerSize = { width: 22.5, height: 16.5 }; // inches
      const printerSize = { x: 256, y: 256, z: 256 }; // Bambu Lab A1
    ...
    it('should handle exact grid multiples without spacers', () => {
      ...
      const drawerSize = { width: 420 / INCH_TO_MM, height: 420 / INCH_TO_MM }; // Convert to inches
    ...
    it('should split large baseplates to fit printer bed', () => {
      ...
      const drawerSize = { width: 12, height: 12 }; // inches (304.8mm x 304.8mm)
      const printerSize = { x: 220, y: 220, z: 250 }; // Smaller printer
    ...
    it('should generate correct spacers for small remainders', () => {
      ...
      const drawerSize = { width: 3.5, height: 3.5 }; // inches (88.9mm x 88.9mm)
    ...
    it('should handle half-size bins when enabled', () => {
      const drawerSize = { width: 5, height: 5 }; // inches
      const printerSize = { x: 200, y: 200, z: 200 };
    ...
    it('should prefer half-size bins for spacers when preferHalfSize is true', () => {
      const drawerSize = { width: 4, height: 4 }; // inches
      const printerSize = { x: 200, y: 200, z: 200 };
    ...
    it('should optimize baseplate selection for printer bed', () => {
      ...
      const drawerSize = { width: 15, height: 15 }; // inches
      const smallPrinter = { x: 180, y: 180, z: 180 }; // Can fit max 4x4 baseplates
      const largePrinter = { x: 350, y: 350, z: 350 }; // Can fit max 8x8 baseplates
      ...
      Object.keys(resultSmall.baseplates).forEach(size => {
        const [gridX, gridY] = size.split('x').map(Number);
      ...
      Object.keys(resultLarge.baseplates).forEach(size => {
        const [gridX, gridY] = size.split('x').map(Number);
        if (gridX > 4 || gridY > 4) {
  ...
  describe('Layout positioning verification', () => {
    ...
    it('should fill space efficiently without gaps', () => {
      const drawerSize = { width: 8, height: 6 }; // inches
      const printerSize = { x: 256, y: 256, z: 256 };
