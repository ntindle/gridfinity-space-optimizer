...
export interface CustomPrinterState {
  ...
  handleInputChange: (axis: keyof PrinterSize, value: string) => void;
  validateAll: () => boolean;
  resetToDefault: () => void;
}
 ...
 */
export const useCustomPrinter = (useMm: boolean = false): CustomPrinterState => {
  // Persisted custom printer dimensions (stored in mm)
  const [customDimensions, setCustomDimensions] = usePersistedState<PrinterSize>(
  ...
  const handleInputChange = (axis: keyof PrinterSize, value: string) => {
    setInputValues(prev => ({
      ...prev,
      [axis]: value,
    }));
    ...
    if (!isNaN(numericValue) && numericValue > 0) {
      ...
      if (validation.valid) {
        ...
        setErrors(prev => ({
          ...prev,
          [axis]: validation.error || 'Invalid dimension',
        }));
      }
    }
  };
